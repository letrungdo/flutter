...
const char* kGraphicsErrorTitle = "HYPER SBI FX";
...
void LogEGLError(std::string_view message) {
  ...
  if (error == EGL_CONTEXT_LOST) {
    ...
    msg << "予期せぬエラーが発生しました。\n\n";
    msg << "Error location: " << message << "\n\n";
    msg << "OKボタンを選択するとアプリケーションが終了します。";
    ...
    __try {
      ...
      if (parent_window != NULL) {
        int result = MessageBoxA(parent_window, msg.str().c_str(),
                    kGraphicsErrorTitle,
      ...
      if (!dialog_shown_successfully) {
        ...
        int result = MessageBoxA(desktop, msg.str().c_str(),
                    kGraphicsErrorTitle,
      ...
      if (!dialog_shown_successfully) {
        ...
        MessageBoxA(NULL, msg.str().c_str(),
                   kGraphicsErrorTitle,
